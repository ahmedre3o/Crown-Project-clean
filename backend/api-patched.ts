...
import { DEFAULT_GEMINI_MODEL, buildGoogleGenAIOptions } from './geminiConfig';
...
const genAI = (() => {
  ...
  try {
    return new GoogleGenAI(buildGoogleGenAIOptions(GEMINI_API_KEY));
  } catch (error) {
...
const normalizePosPaymentMethod = (value: unknown): string => {
  ...
  if (v === 'bank' || v === 'transfer' || v === 'bank_transfer' || v === 'wire' || v === 'تحويل' || v === 'بنكي') return 'card';
...
const createSaleAndItems = async (req: any, paymentMethodOverride?: string) => {
  ...
  try {
    ...
    const pm = normalizePosPaymentMethod(paymentMethodOverride || paymentMethod || 'cash');
