          ...
          {user?.role === 'super_admin' && (
            ...
            <div className="mt-10 border-t border-cyan-500/20 pt-6">
              <h2 className="text-lg font-bold text-cyan-200 mb-2">
                {language === 'ar' ? 'أكواد الباقات والإضافات' : 'Plan & add-on codes'}
              </h2>
              <p className="text-sm text-slate-400 mb-3">
                {language === 'ar'
                  ? 'توليد أكواد الباقات وأكواد إضافة الذكاء الاصطناعي / OCR موجود في صفحة قائمة الأكواد.'
                  : 'Generate subscription plan codes and AI/OCR add-on codes from the Codes List page.'}
              </p>
              <Link
                href="/admin/codes"
                className="inline-flex px-4 py-2 rounded-lg bg-cyan-600 text-white font-semibold text-sm hover:bg-cyan-500"
              >
                {language === 'ar' ? 'فتح قائمة الأكواد وتوليد الأكواد' : 'Open codes list & generators'}
              </Link>
            </div>
          </>
          )}
