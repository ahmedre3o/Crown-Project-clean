...
const normalizeHeader = (value)=>value.toString().trim().toLowerCase().replace(/[_\-]/g, ' ').replace(/\s+/g, ' ');
const detectFields = (headers)=>{
    const map = {};
    const unmatched = [];
    headers.forEach((header, index)=>{
        const normalized = normalizeHeader(header);
        const match = Object.keys(fieldMatchers).find((field)=>fieldMatchers[field].some((key)=>normalized.includes(normalizeHeader(key))));
        if (match) {
            map[index] = match;
        } else if (normalized) {
...
const inferFields = (rows)=>{
    ...
    sample.forEach((row)=>{
        row.forEach((cell, index)=>{
            ...
            if (isNumeric) {
                stats[index].numericCount += 1;
                stats[index].avg += numeric;
                if (Number.isInteger(numeric)) stats[index].integerCount += 1;
                if (/^\d{8,15}$/.test(value)) stats[index].barcodeCount += 1;
            } else {
                stats[index].textCount += 1;
            }
        });
    });
    stats.forEach((stat)=>{
        if (stat.numericCount) stat.avg = stat.avg / stat.numericCount;
    });
    const map = {};
    const nameIndex = stats.map((stat, index)=>({
            index,
            score: stat.textCount
        })).sort((a, b)=>b.score - a.score)[0]?.index;
    if (nameIndex !== undefined) {
        map[nameIndex] = 'name';
    }
    const barcodeIndex = stats.map((stat, index)=>({
            index,
            score: stat.barcodeCount
        })).sort((a, b)=>b.score - a.score)[0]?.index;
    if (barcodeIndex !== undefined && stats[barcodeIndex].barcodeCount > 0) {
        map[barcodeIndex] = 'barcode';
    }
    const numericColumns = stats.map((stat, index)=>({
            index,
            avg: stat.avg,
            numeric: stat.numericCount
        })).filter((stat)=>stat.numeric > 0 && (!map[stat.index] || map[stat.index] !== 'barcode')).sort((a, b)=>b.avg - a.avg);
    if (numericColumns[0]) {
        map[numericColumns[0].index] = 'sellPrice';
    }
    if (numericColumns[1]) {
        map[numericColumns[1].index] = 'buyPrice';
    }
    const quantityColumn = stats.map((stat, index)=>({
            index,
            score: stat.integerCount,
            avg: stat.avg
        })).filter((stat)=>stat.score > 0 && !map[stat.index]).sort((a, b)=>a.avg - b.avg)[0];
    if (quantityColumn) {
        map[quantityColumn.index] = 'stockQuantity';
    }
    return map;
};
function ExcelImportPage() {
    ...
    const handleFile = async (file)=>{
        ...
        try {
            ...
            let { map, unmatched } = detectFields(headers);
            if (Object.keys(map).length === 0) {
                map = inferFields(rows.slice(1));
                unmatched = [];
            }
            const items = [];
            const skipped = [];
            rows.slice(1).forEach((row, rowIndex)=>{
                ...
                if (!rowData.name && !rowData.nameAr) {
                    skipped.push({
                        row: rowIndex + 2,
                ...
                items.push({
                    name: rowData.name || rowData.nameAr,
            ...
            if (items.length === 0) {
                throw new Error('No recognizable product rows found');
            }
            const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$contexts$2f$AuthContext$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["apiRequest"])('/products/import', {
                method: 'POST',
                body: JSON.stringify({
                    items
                })
            ...
            const warnings = unmatched.length ? [
                language === 'ar' ? `أعمدة غير معروفة تم تخطيها: ${unmatched.join(', ')}` : `Unrecognized columns skipped: ${unmatched.join(', ')}`
            ] : [];
            setReport({
                        ...
                        children: [
                            ...
                            report && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                children: [
                                    ...
                                    report.warnings.map((warning, idx)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
...
//# sourceMappingURL=app_f656f44b._.js.map