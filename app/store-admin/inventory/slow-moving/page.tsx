...
function tiedValueFor(it: SlowMovingItem): number {
  if (typeof it.tiedValue === 'number' && Number.isFinite(it.tiedValue)) return it.tiedValue;
  return Number(it.price ?? 0) * Number(it.stock ?? 0);
}
...
export default function SlowMovingPage() {
  ...
  const [type, setType] = useState<'all' | 'dead' | 'slow' | 'near_expiry'>('all');
  ...
  useEffect(() => {
    ...
    if (qType === 'near_expiry') setType('near_expiry');
  }, []);
    ...
    if (type === 'near_expiry') {
      const rows: string[][] = [
        ...
        ...items.map((it) => [
          ...
          it.expiryDate ? new Date(it.expiryDate).toISOString().slice(0, 10) : '-',
          String(it.daysToExpiry ?? ''),
          formatCurrency(tiedValueFor(it), language === 'ar' ? 'ar' : 'en', currency, symbol),
          language === 'ar' ? it.recommendationAr : it.recommendationEn,
        ]),
      ];
      exportToCSV(rows, `near-expiry-stock.csv`);
      return;
    }
    const rows: string[][] = [
  ...
  const handlePrint = async () => {
    await logPrintAudit('slow_moving_stock', 0, branchCtx.activeBranchId ?? undefined);
    window.print();
  };
      ...
      <div className="flex-1 p-8 pt-20 md:pt-8 overflow-y-auto">
        ...
        {summary?.ok && (
          <div className="p-6 neon-card rounded-xl border border-amber-500/40 mb-6">
            ...
            <div className="flex flex-wrap gap-6">
              ...
              {(summary.nearExpiryCount ?? 0) > 0 && (
                <span className="text-xl font-bold text-orange-400">
                  {language === 'ar' ? 'قريب الانتهاء' : 'Near expiry'}: {summary.nearExpiryCount}
                  {typeof summary.nearExpiryValue === 'number' && (
                    <span className="text-base font-normal text-cyan-300 ml-2">
                      ({formatCurrency(summary.nearExpiryValue, language === 'ar' ? 'ar' : 'en', currency, symbol)})
                    </span>
                  )}
                </span>
              )}
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6 print:hidden">
          <div className="flex gap-2 flex-wrap">
            {(['all', 'dead', 'slow', 'near_expiry'] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setType(tab)}
                ...
                {tab === 'all'
                  ...
                  : tab === 'dead'
                    ...
                    : tab === 'slow'
                      ...
                      : (language === 'ar' ? 'قريب الانتهاء' : 'Near expiry')}
              </button>
            ))}
          </div>
          {type !== 'near_expiry' && (
          ...
          >
            ...
            {Object.entries(BUCKET_LABELS).filter(([k]) => k !== 'near_expiry').map(([k, v]) => (
        ...
        {/* Table */}
        <div className="p-6 neon-card rounded-xl accounting-print">
          <div className="hidden print:block print-branch text-black border-b border-gray-300 pb-3 mb-4">
            ...
            <div>{type === 'near_expiry' ? (language === 'ar' ? 'قريب الانتهاء' : 'Near expiry') : (language === 'ar' ? 'الراكد/البطيء' : 'Dead/Slow Stock')}</div>
            <div>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-gray-500">
              {type === 'near_expiry'
                ? (language === 'ar' ? 'لا توجد منتجات قريبة من الانتهاء (30 يوماً)' : 'No products expiring within 30 days')
                : (language === 'ar' ? 'لا توجد منتجات راكدة/بطيئة' : 'No dead/slow products')}
            </div>
          ) : type === 'near_expiry' ? (
            <div className={`table-scroll table-rtl-wrap ${direction === 'rtl' ? 'text-right' : 'text-left'}`} dir={direction}>
              <table className="data-table text-sm text-slate-200 min-w-[900px]">
                <thead>
                  <tr>
                    <th className="col-center w-10 print-hide-col"> </th>
                    <th className="table-col-long">{language === 'ar' ? 'المنتج' : 'Product'}</th>
                    <th className="table-col-compact">SKU</th>
                    <th className="table-col-compact">{language === 'ar' ? 'المخزون' : 'Stock'}</th>
                    <th className="table-col-date">{language === 'ar' ? 'تاريخ الانتهاء' : 'Expiry'}</th>
                    <th className="table-col-compact">{language === 'ar' ? 'أيام' : 'Days'}</th>
                    <th className="table-col-currency">{language === 'ar' ? 'القيمة المربوطة' : 'Tied Value'}</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((it) => (
                    <React.Fragment key={it.productId}>
                      ...
                      >
                        <td className="data-table-center print-hide-col">{expandedId === it.productId ? <ChevronDown className="h-4 w-4 mx-auto" /> : <ChevronRight className="h-4 w-4 mx-auto" />}</td>
                        <td className="table-col-long">{language === 'ar' ? it.nameAr || it.name : it.name}</td>
                        <td className="data-table-num table-col-compact font-mono text-slate-400">{it.sku || '-'}</td>
                        <td className="data-table-num table-col-compact">{it.stock}</td>
                        <td className="data-table-num table-col-date text-slate-400">
                          <span dir="ltr" className="tabular-nums">{it.expiryDate ? new Date(it.expiryDate).toISOString().slice(0, 10) : '-'}</span>
                        </td>
                        <td className="data-table-num table-col-compact text-orange-300">{it.daysToExpiry ?? '-'}</td>
                        <td className="data-table-num table-col-currency font-semibold text-cyan-300">
                          <span dir="ltr" className="tabular-nums">{formatCurrency(tiedValueFor(it), 'en', currency, symbol)}</span>
                        </td>
                      </tr>
                      {expandedId === it.productId && (
                        <tr className="bg-gray-800/30">
                          <td colSpan={7} className="py-4 px-4 table-col-multiline">
            ...
            <table className="data-table text-sm text-slate-200 min-w-[1200px]">
              <thead>
                <tr>
                  ...
                  <th className="table-col-compact">{language === 'ar' ? 'المخزون' : 'Stock'}</th>
                  <th className="table-col-currency">{language === 'ar' ? 'القيمة المربوطة' : 'Tied Value'}</th>
                  <th className="table-col-date">{language === 'ar' ? 'آخر بيع' : 'Last Sold'}</th>
                  <th className="table-col-compact">{language === 'ar' ? 'الكمية المباعة' : 'Sold (window)'}</th>
                  <th className="table-col-date">{language === 'ar' ? 'الفترة' : 'Bucket'}</th>
                </tr>
              </thead>
              <tbody>
                {items.map((it) => (
                  <React.Fragment key={it.productId}>
                    ...
                    >
                      ...
                      <td className="data-table-num table-col-date text-slate-400">
                        <span dir="ltr" className="tabular-nums">{it.lastSoldAt ? new Date(it.lastSoldAt).toISOString().slice(0, 10) : '-'}</span>
                      </td>
                      <td className="data-table-num table-col-compact">{it.soldQtyWindow ?? '-'}</td>
                      <td className="table-col-date">{BUCKET_LABELS[it.bucket] ? (language === 'ar' ? BUCKET_LABELS[it.bucket].ar : BUCKET_LABELS[it.bucket].en) : it.bucket}</td>
                    </tr>
                    {expandedId === it.productId && (
                      <tr className="bg-gray-800/30">
                        <td colSpan={8} className="py-4 px-4 table-col-multiline">
                          <div className="text-sm">
                            <p className="text-cyan-300 mb-1">{language === 'ar' ? 'التوصية' : 'Recommendation'}:</p>
                            <p>{language === 'ar' ? it.recommendationAr : it.recommendationEn}</p>
