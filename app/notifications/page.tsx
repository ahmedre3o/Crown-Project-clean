...
import {
  ...
  formatNotificationSummaryLine,
  formatPayloadRows,
  getNotificationPayloadRecord,
...
export default function NotificationsPage() {
  ...
  const meta = (n: Notification): { orderId?: number; invoiceId?: number; saleId?: number } => {
    const o = getNotificationPayloadRecord(n, language);
    return {
      orderId: o.orderId != null ? Number(o.orderId) : undefined,
      invoiceId:
        o.invoiceId != null ? Number(o.invoiceId) : o.saleId != null ? Number(o.saleId) : undefined,
      saleId: o.saleId != null ? Number(o.saleId) : undefined,
    };
  };
  ...
  return (
    <div
      className={`min-h-screen h-[100dvh] overflow-y-auto overflow-x-hidden bg-[#020617] ${direction === 'rtl' ? 'font-[system-ui]' : ''} ${
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 md:py-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-3 mb-6">
          <div className="flex-1 min-w-[200px] relative">
            <Search className="absolute top-1/2 -translate-y-1/2 start-3 h-4 w-4 text-slate-500 pointer-events-none" />
            <input
              type="text"
              value={search}
              ...
              className="w-full h-11 ps-10 pe-4 rounded-xl border border-cyan-500/25 bg-black/30 text-slate-100 placeholder:text-slate-500 text-sm"
            />
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {items.map((n) => {
              ...
              const summaryLine = formatNotificationSummaryLine(n, language);
              const payloadRows = formatPayloadRows(getNotificationPayloadRecord(n, language), language);
              return (
              <div
                key={n.id}
                id={`notif-${n.id}`}
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {summaryLine ? (
                      <div className="text-xs text-slate-400 mt-1.5 whitespace-pre-wrap break-words leading-relaxed">
                        {summaryLine}
                      </div>
                    ) : null}
                ...
                {expandedId === n.id && (
                  <div className="border-t border-cyan-500/15 px-4 md:px-5 py-3 md:py-4 bg-black/20">
                    {payloadRows.length > 0 && (
                      <div
                        className="max-h-[40vh] overflow-y-auto rounded-xl border border-cyan-500/15 bg-black/25 p-3 mb-3 space-y-2"
                        style={{ WebkitOverflowScrolling: 'touch' } as React.CSSProperties}
                      >
                        {payloadRows.map((row) => (
                          <div
                            key={row.key}
                            className="flex flex-col sm:flex-row sm:items-baseline gap-0.5 sm:gap-3 pb-2 border-b border-cyan-500/10 last:border-0 last:pb-0"
                          >
                            <span className="text-[11px] uppercase tracking-wide text-slate-500 shrink-0 sm:min-w-[7rem]">
                              {row.label}
                            </span>
                            <span className="text-sm text-slate-200 font-medium break-all">{row.value}</span>
                          </div>
                        ))}
                      </div>
                    )}
