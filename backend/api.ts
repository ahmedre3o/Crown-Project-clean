...
const fieldMatchers: Record<string, string[]> = {
  ...
  // Accept QR_Code style headers and map them to barcode to support scanner guns.
  ...
  sellPrice: ['sell', 'price', 'sale', 'sellprice', 'saleprice', 'unitprice', 'السعر', 'سعر البيع'],
...
const validateProductImportMapping = (columnMap: Record<string, string>): ImportMappingValidation => {
  ...
  const hasName = mappedFields.has('name') || mappedFields.has('nameAr');
  const hasSellOrBuyPrice = mappedFields.has('sellPrice') || mappedFields.has('buyPrice');
  const missingFields: Array<'name' | 'sellPrice'> = [];
  if (!hasName) missingFields.push('name');
  if (!hasSellOrBuyPrice) missingFields.push('sellPrice');
...
const buildProductImportMappingGuide = (headers: string[], columnMap: Record<string, string>, reason: string) => {
  ...
  return {
    ...
    requiredFields: [
      {
        ...
        acceptedHeaders: fieldMatchers.name,
      },
      {
        field: 'sellPrice',
        note: 'Sell price is required (or provide Buy Price and we will calculate Sell Price).',
        acceptedHeaders: [...fieldMatchers.sellPrice, ...fieldMatchers.buyPrice],
      },
    ],
    suggestedHeaders: {
    ...
    tip: 'Rename your Excel/CSV column headers to match one of the accepted headers above, then re-upload.',
  };
};
...
const isHeaderLike = (headers: string[]) => {
  // Accept common "Crown import" headers explicitly.
  const normalizedHeaders = headers.map((header) => normalizeHeader(header));
  const crownHeaders = new Set(['part name', 'brand', 'qr code', 'buyprice', 'stock', 'image url']);
  if (normalizedHeaders.some((h) => crownHeaders.has(h))) {
    return true;
  }

  const joined = headers.map((header) => normalizeHeader(header)).join(' ');
  const matchers = Object.values(fieldMatchers).flat();
  const hasKnownToken = matchers.some((token) => joined.includes(normalizeHeader(token)));
...
const heuristicColumnMap = (headers: string[]) => {
  ...
  headers.forEach((header) => {
    ...
    const match = Object.keys(fieldMatchers).find((field) =>
      fieldMatchers[field].some((key) => normalized.includes(normalizeHeader(key)))
    );
    if (match) {
      map[header] = match;
    }
...
app.post(
  ...
  try {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      const name = normalizeText(item.name || item.nameEn || item.nameAr);
      const sku = normalizeText(item.sku);
      const barcode = normalizeText(item.barcode);
      const qrCode = normalizeText(item.qrCode);

      if (!name) {
        skipped.push({ row: index + 1, reason: 'Missing product name' });
        continue;
      }

      if (sku && skuSet.has(sku)) {
        skipped.push({ row: index + 1, reason: 'Duplicate SKU' });
        continue;
      }
      if (barcode && barcodeSet.has(barcode)) {
        skipped.push({ row: index + 1, reason: 'Duplicate barcode' });
        continue;
      }
      if (qrCode && qrSet.has(qrCode)) {
        skipped.push({ row: index + 1, reason: 'Duplicate QR code' });
        continue;
      }
      if (nameSet.has(name)) {
        skipped.push({ row: index + 1, reason: 'Duplicate name' });
        continue;
      }

      const sellPrice = parseFloat(item.sellPrice || item.price || '0');
      const buyPrice = parseFloat(item.buyPrice || item.cost || '0');
      ...
      if (!sellPrice) {
        skipped.push({ row: index + 1, reason: 'Missing sell price' });
        continue;
      }

      const nameValue = item.nameEn || item.nameAr || item.name || name;
      const nameArValue = item.nameAr || item.nameEn || item.name || name;
    ...
    res.status(201).json({ importedCount: imported.length, skipped, imported });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
  }
...
app.post(
  ...
  try {
    ...
    const queryModeRaw = String((req as any)?.query?.mode || '').trim();
    const modeLocked = Boolean(queryModeRaw);
    ...
    const [categories] = await pool.execute('SELECT id, name_en, name_ar FROM categories WHERE shop_id = ? OR shop_id IS NULL', [
      shopId,
    ]);
    ...
    const safeParseClientColumnMap = (raw: string) => {
      try {
        ...
        Object.entries(parsed).forEach(([header, field]) => {
          const h = String(header || '').trim();
          const f = String(field || '').trim();
          if (!h || !f) return;
          if (!allowedImportFields.has(f)) return;
          out[h] = f;
        });
        return out;
      } catch {
    ...
    const mapAndInsert = async (
      ...
      const skipRow = (reason: string) => {
        if (isAnalyze) {
          if (reason === 'Missing product name') {
            analysis.missingNameCount += 1;
            if (issueSamples.missingName.length < 5) issueSamples.missingName.push({ row: rowIndex });
          } else if (reason === 'Duplicate SKU') {
            analysis.duplicateSkuCount += 1;
          } else if (reason === 'Duplicate barcode') {
            analysis.duplicateBarcodeCount += 1;
          } else if (reason === 'Duplicate name') {
            analysis.duplicateNameCount += 1;
      ...
      // NOTE: importedCount includes flushed batches; batch.length are pending inserts.
      ...
      const name = normalizeText(mapped.name || mapped.nameAr);
      if (!name) {
        skipRow('Missing product name');
        return;
      }

      // ===== Custom mapping rules for Crown imports =====
      // Part_Name -> mapped.name (handled via header matcher)
      // QR_Code -> Barcode/SKU
      ...
      // If QR_Code is provided, treat it as a universal code (barcode/sku).
      if (!barcodeRaw && qrRaw) barcodeRaw = qrRaw;
      if (!skuRaw && (barcodeRaw || qrRaw)) skuRaw = barcodeRaw || qrRaw;

      const sku = skuRaw ? normalizeText(skuRaw) : '';
      const barcode = barcodeRaw ? normalizeText(barcodeRaw) : '';

      if (sku && skuSet.has(sku)) {
        skipRow('Duplicate SKU');
        return;
      }
      if (barcode && barcodeSet.has(barcode)) {
        skipRow('Duplicate barcode');
        return;
      }
      if (nameSet.has(name)) {
        skipRow('Duplicate name');
        return;
      }
      ...
      const missingPrice = !hasSell && !hasBuy;
      ...
      if (hasSell) {
        ...
        analysis.missingPriceCount += 1;
        if (issueSamples.missingPrice.length < 5) {
          issueSamples.missingPrice.push({ row: rowIndex, value: String(mapped.name || mapped.nameAr || '').slice(0, 60) });
        }
      }

      if (!missingPrice) {
        analysis.readyRows += 1;
      }

      // Import-time policy for rows with missing prices (PowerQuery-like "skip or fix")
      if (mode === 'import' && missingPrice) {
        if (missingPricePolicy === 'skip') {
          skipRow('Missing sell price');
          return;
        }
        if (missingPricePolicy === 'default') {
          const d = defaultSellPrice ?? 0;
          if (d > 0) {
            sellPrice = d;
          } else {
            skipRow('Missing sell price');
      ...
      const normalizedBuyPrice = buyPriceRaw ?? 0;
      const stockQuantity = Math.max(0, Math.floor(normalizeNumber(mapped.stockQuantity ?? mapped.quantity ?? 0) || 0));
      const minStockLevel = Math.max(1, Math.floor(normalizeNumber(mapped.minStockLevel ?? 5) || 5));
      ...
      if (mode === 'import') {
        batch.push([
          mapped.name || mapped.nameAr,
          mapped.nameAr || mapped.name,
          skuRaw || null,
          barcodeRaw || null,
          null,
          mapped.brand || null,
        ...
        console.log(
          `${logPrefix} queued: name="${String(mapped.name || mapped.nameAr || '').trim().slice(0, 60)}" sku="${skuRaw}" barcode="${barcodeRaw}"`
        );
        ...
        // analyze mode: do not write to DB
        claimIdentity();
      }
    };
    ...
    const finish = async () => {
        ...
        const defaultGuide =
          !mappingValidation.ok || analyzeError
            ? buildProductImportMappingGuide(
                headers,
                columnMap,
                analyzeError || 'Missing required fields in header mapping'
              )
            : null;
        return res.json({
          ok: !analyzeError,
      ...
      if (mappingInvalid) {
        return res.status(400).json({
          error:
            'Unrecognized inventory file format. Please rename your column headers to match the required fields and try again.',
          mappingGuide:
            mappingGuide ||
            buildProductImportMappingGuide(headers, columnMap, 'No recognizable mapping for required fields'),
          ...payload,
        });
      }
    ...
    busboy.on('file', async (_name: string, file: NodeJS.ReadableStream, info: any) => {
      fileFound = true;
      ...
      if (lower.endsWith('.csv')) {
        ...
        let rowIndex = 1;
        parser.on('headers', async (parsedHeaders) => {
          headers = parsedHeaders as string[];
          file.pause();
          const headerLike = isHeaderLike(headers);
          if (!headerLike) {
            columnMap = {};
            unmappedColumns = headers;
            mappingInvalid = true;
            analyzeError = 'No header row detected';
            mappingGuide = buildProductImportMappingGuide(headers, columnMap, 'No header row detected');
            await finish();
            file.resume();
            return;
          } else {
            const heuristicMap = heuristicColumnMap(headers);
            const aiMap = await aiColumnMap(headers);
            const autoMap = mergeColumnMaps(heuristicMap, aiMap);
            columnMap = clientColumnMap ? clientColumnMap : autoMap;
            unmappedColumns = headers.filter((header) => !columnMap[header]);
            const validation = validateProductImportMapping(columnMap);
            mappingValidation = validation;
            mappingInvalid = mode === 'import' && !validation.ok;
            if (mode === 'analyze' && !validation.ok) {
              mappingGuide = buildProductImportMappingGuide(headers, columnMap, 'Missing required fields in CSV header mapping');
            }
            if (mappingInvalid) {
              mappingGuide = buildProductImportMappingGuide(
                headers,
                columnMap,
                'Missing required fields in CSV header mapping'
              );
              await finish();
              file.resume();
              return;
        ...
        parser.on('data', async (row) => {
          if (processDone) return;
          rowIndex += 1;
          updateProfile(headers, row);
          collectSamples(headers, row);
          file.pause();
          try {
            await mapAndInsert(row, headers, columnMap, rowIndex);
          } finally {
            file.resume();
          }
        });
      ...
      const workbookReader = new ExcelJS.stream.xlsx.WorkbookReader(file as any, {
        entries: 'emit',
        sharedStrings: 'cache',
        worksheets: 'emit',
      });
      ...
      for await (const worksheet of workbookReader) {
        ...
        for await (const row of worksheet) {
          ...
          if (rowIndex === 1) {
            ...
            mappingInvalid = mode === 'import' && !validation.ok;
            if (mode === 'analyze' && !validation.ok) {
              mappingGuide = buildProductImportMappingGuide(headers, columnMap, 'Missing required fields in Excel header mapping');
            }
            if (mappingInvalid) {
              mappingGuide = buildProductImportMappingGuide(headers, columnMap, 'Missing required fields in Excel header mapping');
              await finish();
