...
export function BranchProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadBranches = useCallback(async () => {
    ...
    // Load branches for any authenticated user with a shop (including client accounts / Bronze-Silver-Gold).
    setLoading(true);
    try {
      ...
      if (list.length === 0) {
        setActiveBranchIdState(null);
        if (typeof window !== 'undefined') localStorage.removeItem(`crown-active-branch-${shopId}`);
      } else {
  ...
  }, [shopId, user?.role, user?.package, pathname]);
