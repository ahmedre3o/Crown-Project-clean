...
function TrackPageContent() {
  ...
  const hasQuery = Boolean(code.trim() && phone.trim());
  ...
  const backToStoreHref = (() => {
    ...
    if (shopIdParam && /^\d+$/.test(shopIdParam)) return `/storefront?shopId=${encodeURIComponent(shopIdParam)}`;
    return '/storefront?shopId=1';
  })();

  const submitTrackForm = (e: React.FormEvent) => {
    e.preventDefault();
    const c = formCode.trim();
    const p = formPhone.trim();
    if (!c || !p) {
      setError(lang === 'ar' ? 'أدخل كود التتبع ورقم الهاتف' : 'Enter tracking code and phone');
      return;
    }
    const q = new URLSearchParams();
    q.set('code', c);
    q.set('phone', p);
    if (shopIdParam) q.set('shopId', shopIdParam);
  ...
  return (
    <div className="min-h-screen bg-[#06070b] text-white" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {!hasQuery ? (
          <form
            onSubmit={submitTrackForm}
            className="rounded-2xl border border-cyan-500/25 bg-black/35 p-6 space-y-4"
          >
            <p className="text-sm text-slate-400">
              {lang === 'ar'
                ? 'أدخل كود التتبع ورقم الهاتف المستخدمين عند الطلب.'
                : 'Enter the tracking code and phone number used when ordering.'}
            </p>
            {error && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-amber-100 text-sm">{error}</div>
            )}
            <div>
              <label className="block text-xs text-slate-400 mb-1">{lang === 'ar' ? 'كود التتبع' : 'Tracking code'}</label>
              <input
                value={formCode}
                onChange={(e) => setFormCode(e.target.value)}
                className="w-full h-11 px-3 rounded-xl border border-cyan-500/25 bg-black/40 text-slate-100"
                placeholder="DN2M5P"
                autoComplete="off"
              />
            </div>
            <div>
              <label className="block text-xs text-slate-400 mb-1">{lang === 'ar' ? 'رقم الهاتف' : 'Phone'}</label>
              <input
                value={formPhone}
            ...
            <button
              type="submit"
              className="w-full h-11 rounded-xl bg-gradient-to-r from-cyan-600 to-fuchsia-600 text-white font-semibold"
            >
